...
\begin{itemize}
    ...
    \item \textbf{Load data:} One file for load real power and one for reactive power. The files need to be in CSV format. An example of the format for the 9-bus case is \href{https://github.com/ORNL/ExaGO/tree/develop/datafiles/case9}{here}.
    \item \textbf{Wind generation:} The wind generation time-series described in CSV format. See an example of the format \href{https://github.com/ORNL/ExaGO/tree/develop/datafiles/case9}{here}.
\end{itemize}
...
Number of steps                     7
Active power demand profile         ExaGO/datafiles/case9/load_P.csv
Rective power demand profile        ExaGO/datafiles/case9/load_Q.csv
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{0.4\textwidth}|p{0.35\textwidth}|p{0.25\textwidth}|}
    \hline
    \textbf{Option} & \textbf{Description} & \textbf{Default / Notes} \\ \hline
    -netfile $<$path$>$ & MATPOWER network file & \href{https://github.com/ORNL/ExaGO/tree/develop/datafiles/case9/case9mod.m}{case9mod.m} \\ \hline
    -tcopflow\_ploadprofile $<$csv$>$ & Active power demand profile & Flat profile if omitted \\ \hline
    -tcopflow\_qloadprofile $<$csv$>$ & Reactive power demand profile & Flat profile if omitted \\ \hline
    -tcopflow\_windgenprofile $<$csv$>$ & Wind generation profile & Flat profile (base dispatch) if omitted \\ \hline
    -tcopflow\_dT $<$minutes$>$ & Time-step length in minutes & 5.0 \\ \hline
    -tcopflow\_duration $<$hours$>$ & Simulation horizon in hours & 0.5 (30 minutes) \\ \hline
    -tcopflow\_iscoupling $<$0/1$>$ & Enable/disable inter-temporal coupling (generator ramping) & 1 (enabled) \\ \hline
    -tcopflow\_tolerance $<$value$>$ & Optimality tolerance passed to the solver & Inherits IPOPT default if not set \\ \hline
    -tcopflow\_model $<$name$>$ & Temporal model & GENRAMP \\ \hline
    -tcopflow\_solver $<$name$>$ & Nonlinear solver backend & IPOPT \\ \hline
    -print\_output & Dump solution summary to stdout & Disabled by default \\ \hline
    -save\_output & Write MATPOWER files for each step into tcopflowout/ & Disabled by default \\ \hline
    -opflow\_ignore\_lineflow\_constraints $<$0/1$>$ & Forwarded to each embedded OPFLOW instance to toggle thermal limits & 0 \\ \hline
    -options\_left no & PETSc utility flag to suppress unused-option warnings & Optional \\ \hline
  \end{tabular}
